...
export class VercelProvider {
    ...
    async getStatus(deploymentId) {
        try {
            const response = await axios.get(`${this.apiUrl}/v13/deployments/${deploymentId}`, {
                headers: {
    ...
    async getDomains(projectId) {
        try {
            const response = await axios.get(`${this.apiUrl}/v9/projects/${projectId}/domains`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
                params: this.teamId ? { teamId: this.teamId } : {},
            });
            return response.data.domains.map((d) => d.name);
        }
        catch (error) {
    ...
    async addDomain(projectId, domain) {
        try {
            await axios.post(`${this.apiUrl}/v10/projects/${projectId}/domains`, { name: domain }, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
    ...
    async removeDomain(projectId, domain) {
        try {
            await axios.delete(`${this.apiUrl}/v9/projects/${projectId}/domains/${domain}`, {
                headers: {
                    Authorization: `Bearer ${this.apiToken}`,
                },
                params: this.teamId ? { teamId: this.teamId } : {},
    ...
    async createDeployment(config) {
        const response = await axios.post(`${this.apiUrl}/v13/deployments`, {
    ...
    async uploadFiles(deploymentId, config) {
        ...
        for (const file of files) {
            await axios.post(`${this.apiUrl}/v2/deployments/${deploymentId}/files`, {
                [file.file]: {
                    data: file.data,
                    encoding: 'utf-8',
                },
