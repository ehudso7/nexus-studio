...
export class NetlifyProvider implements DeploymentProvider {
  ...
  async getStatus(deploymentId: string): Promise<DeploymentStatus> {
    try {
      const response = await axios.get(
        `${this.apiUrl}/deploys/${deploymentId}`,
        {
          headers: {
  ...
  async rollback(deploymentId: string): Promise<void> {
    try {
      ...
      await axios.post(
        `${this.apiUrl}/sites/${siteId}/deploys/${deploymentId}/restore`,
        {},
        {
          headers: {
            Authorization: `Bearer ${this.apiToken}`,
  ...
  private async createOrGetSite(name: string): Promise<any> {
    try {
...
`;
  ...
  private async deployZip(siteId: string, zipPath: string): Promise<any> {
